...
async function main(orgName, userName, id) {
    try {
        ...
        const result = await contract.evaluateTransaction('searchList', id.toString());
        const rr = JSON.parse(result);
    //    const res1 = _.find(rr, (val, index) => {
    //         const a = _.findKey(val, {name:});
    //         return a;
    //     });
        return rr;
        // var parsed = JSON.parse(result);
        // console.log(orgName);
        // if(parsed[id].orgName === orgName) {
        //     return JSON.parse(result);
        // } else {
        //     return `Please get permission from the Organization: ${orgName}`;
        // }
    } catch (error) {
