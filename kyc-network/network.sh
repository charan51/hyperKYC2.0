...
function printHelp() {
  ...
  echo "      - 'deployCC' - deploy the kycDetails chaincode on the channel"
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create CitiBank Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-citiBank.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "##########################################################"
    echo "############ Create KYC Orderer Org Identities ###############"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-KYCorderer.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
  ...
  echo
  echo "Generate CCP files for citiBank and sbi"
...
## call the script to join create the channel and join the peers of citiBank and sbi
...
function networkDown() {
  # stop org3 containers also in addition to citiBank and sbi, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf organizations/fabric-ca/citiBank/msp organizations/fabric-ca/citiBank/tls-cert.pem organizations/fabric-ca/citiBank/ca-cert.pem organizations/fabric-ca/citiBank/IssuerPublicKey organizations/fabric-ca/citiBank/IssuerRevocationPublicKey organizations/fabric-ca/citiBank/fabric-ca-server.db
    ...
    rm -rf channel-artifacts log.txt kycDetails.tar.gz kycDetails
